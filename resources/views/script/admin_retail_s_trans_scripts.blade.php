...
<script>
    $(document).ready(function() {
        ...
        $('input[name="radio-rset"]').change(function() {
            ...
            $.ajax({
                url: "{{ route('admin_transactions.admin_s_transaction.stocks') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (serial_stock_selling == '') {
                        ...
                        Swal.fire({
                            icon: 'info',
                            text: 'No available stock for this currency.',
                            customClass: {
                                popup: 'my-swal-popup',
                            }
                        });
                        ...
                        setTimeout(function() {
                            $('#container-test').fadeOut("fast");
                            $('#rate-used-selling').removeAttr('disabled', 'disabled');

                            serial_stock_selling.forEach(function(gar) {
                                sellingBills(gar.BillAmount, gar.Currency, gar.ID, gar.Rset, gar.Serials, gar.source_type, items_length);

                                available_bills_total_amnt += gar.BillAmount;
                            });
                            ...
                            rate_used_selling.forEach(function(gar) {
                                rateSelling(gar.CurrencyID, gar.SinagRateSelling);
                            });
        ...
        function sellingBills(BillAmount, Currency, ID, Rset, Serials, source_type, items_length) {
            var table = $('#serial-stock-table-appended');
            var new_row_selling = $('<tr class="data-serials-tr" data-serials="'+ Serials +'" data-billamount="'+ BillAmount +'">');
            var action_cell = $('<td class="text-center text-sm p-1"><input class="form-check-input serials-selling-selected" type="checkbox" name="select-serials-radio-appended" value="'+ Serials +'" data-serialamount="'+ BillAmount.toFixed(2) +'" data-ids="'+ ID +'" data-sourcetype="'+ source_type +'" data-billamount="'+ BillAmount +'" readonly></td>');
            // var serials_cell = $('<td class="text-center text-sm"><span class="serials-appended-span">'+ Serials +'</span> Rset: <strong>('+ Rset +')</strong></td>');
            var serials_cell = $('<td class="text-center text-sm p-1"><span class="serials-appended-span">'+ Serials +'</span> </td>');
            var bill_amount_cell = $('<td class="text-right text-sm py-1 px-3">'+ BillAmount.toLocaleString("en" , {minimumFractionDigits: 2 , maximumFractionDigits: 2}) +'</td>');
            ...
            if (items_length > 10) {
                $('#serial-stock-container').css({
                    height: items_length * 10
                });

                // table_height = items_length * 10;
            } else if (items_length < 10) {
                $('#serial-stock-container').css({
                    height: 'auto'
                });
            }
        ...
        function rateSelling(CurrencyID, SinagRateSelling) {
            $('#rate-used-selling').val(SinagRateSelling);
            $('#rate-used-selling-curr-id').val(CurrencyID);
            $('#rate-used-true').val(SinagRateSelling);
        }
        ...
        $('body').on('click', '#button-add-serial', function() {
            ...
            $('#select-serial-stock-appended').off("click").on("click", function() {
                ...
                var selected_serial_fsid = $('input[name="select-serials-radio-appended"]:checked').attr('data-ids');
                ...
                if (selected_serial_serial.length === 0) {
                    Swal.fire({
                        icon: 'error',
                        text: 'Select a bill to sell.',
                        customClass: {
                            popup: 'my-swal-popup',
                        }
                    });
                } else if (selected_serial_serial.length > 15) {
                    Swal.fire({
                        icon: 'error',
                ...
                } else if (serial_count + selected_serial_serial.length > 15) {
                    Swal.fire({
                        icon: 'error',
                        text: 'Selected serials limit has been reached.',
                        customClass: {
                    ...
                    Swal.fire({
                        ...
                        $('.serials-selling-selected').each(function() {
                            ...
                            if (checked_items == true) {
                                ...
                                var source_type = $(this).attr('data-sourcetype');
                        ...
                        if (counter <= counter++) {
                            selected_serials_data.forEach(function(gar) {
                                    ...
                                    `<div class="row align-items-center px-3 my-3 new-serial-container" id="new-serial-container">
                                        <div class="col-12">
                                            <div class="row align-items-center">
                                                ...
                                                <div class="col-5">
                                                    <input type="text" class="form-control form-control-sm serial-input-appended-data text-center" name="bill-serial-selling[]" autocomplete="off" placeholder="Select a serial" data-serialinputcount="`+ counter +`" readonly value="`+ gar.serial +`">
                                                    <input type="hidden" class="form-control bill-serial-fsid-selling" name="bill-serial-fsid-selling[]" readonly value="`+ gar.IDS +`" data-source="${gar.source}">
                                                </div>
                                                <div class="col-3">
                                                    <input type="text" class="form-control form-control-sm currency-amnt-selling text-right" name="currency-amnt-selling[]" autocomplete="off" placeholder="0.00" readonly value="`+ gar.bill_amount +`">
                                                    <input type="hidden" class="form-control bill-amnt-selling" name="bill-amnt-selling[]" autocomplete="off" placeholder="0.00" readonly value="`+ gar.bill_amount +`">
                                                </div>
                                                <div class="col-1 text-end">
                                                    <button class="btn btn-danger btn-danger-serial pe-2" id="button-delete-serial" type="button" value="`+ gar.serial +`"><i class='bx bx-trash'></i></button>
                            ...
                            if (decim_part > 0 && decim_part <= 0.25) {
                                decim_part = 0.25;
        ...
        $('body').on('click', '.btn-danger-serial', function deleteSerial() {
            ...
            var rate_used_true = isNaN($('#rate-used-selling').val()) ? 0 : $('#rate-used-selling').val();
            ...
            var total_amnt_selling_delete = parseFloat($('#true-total-amnt-selling') .val());
            var computed_amnt_selling = parseFloat(rate_used_true) * parseFloat(bill_amtn_selling);
        ...
        $('#rate-used-selling').keyup(function() {
            ...
            var final_selling_rate = isNaN(rate_used_keyup_selling) ? 0 : rate_used_keyup_selling;
            // var bill_amnt_selling = $('#bill-amnt-selling').val() == undefined ? 0 : $('#bill-amnt-selling').val();
            var bill_amnt_selling = $('#true-currency-amnt-selling').val() == '' ? 0 : $('#true-currency-amnt-selling').val();
            var new_total_amount = parseFloat(final_selling_rate) * bill_amnt_selling;
            ...
            $('#total-amnt-selling').val(final_t_amnt.toLocaleString("en" , {minimumFractionDigits: 2 , maximumFractionDigits: 2})).text(final_t_amnt.toLocaleString("en" , {minimumFractionDigits: 2 , maximumFractionDigits: 2}));
            $('#true-total-amnt-selling').val(final_t_amnt);
        ...
        $('#rate-used-selling').on('change', function() {
            ...
            if (rate_used_selling_change < get_rate_used_true) {
                Swal.fire({
                    text: "New rate shouldn't be lower than the current rate.",
                    icon: 'warning',
        ...
        if (availalbe_stocks_count == 0) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    var url = "{{ route('admin_transactions.pending_serials') }}";
    ...
    //         var regex = new RegExp(search_serial, 'i');
    ...
    $(document).ready(function() {
        $('#proceed-transaction').click(function() {
            ...
            $('.bill-serial-fsid-selling').each(function() {
                ...
                success: function(get_user_info) {
                    ...
                    if (sec_code_array.includes(user_sec_onpage)) {
                        ...
                        Swal.fire({
                            ...
                            form_data.append('FSIDs', from_branch.join(", "));
                            form_data.append('AFSIDs', from_admin.join(", "));
                            form_data.append('matched_user_id', matched_user_id);

                            $.ajax({
                                url: "{{ route('admin_transactions.admin_s_transaction.save') }}",
                                type: "post",
                                data: form_data,
                                contentType: false,
                                processData: false,
                                cache: false,
                                success: function(data) {
                                    var route = "{{ route('admin_transactions.admin_s_transaction.details', ['id' => ':id']) }}";
                                    var url = route.replace(':id', data.latest_ascid);
