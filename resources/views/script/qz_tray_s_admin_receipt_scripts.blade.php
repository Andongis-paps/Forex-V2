...
<script>
    ...
    $('.print-selling-admin-receipt').click(function() {
        $.ajax({
            url: "{{ route('admin_transactions.bulk_selling.print') }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                company_id: $(this).attr('data-companyid'),
                STMDID: $(this).attr('data-stmdid'),
            },
            // beforeSend: function() {
    ...
    function adminSellingTransDetails() {
        if (notReady()) {
            return;
        }
