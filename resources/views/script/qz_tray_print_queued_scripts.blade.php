...
<script>
    ...
    $('#print-queued-bills').click(function() {
        $.ajax({
            ...
            success: function(gar) {
                ...
                if (gar.test.length <= 0) {
                    Swal.fire({
                        icon: 'warning',
                        html:  `<div><span class="text-sm text-black">No queued bills.</span></div>`,
                        showConfirmButton: true,
                        allowOutsideClick: false,
                        allowEscapeKey: false,
                    });

                    // $('#queued-by-reest-sec-code').prop('disabled', true);
                    // $('#print-q-proceed-transaction').prop('disabled', true);
                } else {
                    // $('#queued-by-reest-sec-code').prop('disabled', false);
                    // $('#print-q-proceed-transaction').prop('disabled', false);
                    $('#queued-by-rset-modal').modal("show");
                    $('#queued-bills-t-body').html(var_test);
                }
            },
            // complete: function() {
            //     setTimeout(function() {
    ...
    //                 Swal.fire({
    //                     icon: 'warning',
    //                     html:  `<div><span class="text-sm text-black">No queued bills for this receipt set.</span></div>`,
    //                     showConfirmButton: true,
    //                     allowOutsideClick: false,
    //                     allowEscapeKey: false,
    //                 });
    ...
    $('#print-q-proceed-transaction').click(function() {
        var user_id_array = [];
        var sec_code_array = [];
            ...
            success: function(get_user_info) {
                ...
                if (sec_code_array.includes(on_page_final_printing)) {
                    ...
                    Swal.fire({
                        ...
                        // },
                        didOpen: () => {
                            // $('input[name="radio-rset"]').prop('disabled', true);
                        ...
                        $('input[name="radio-rset"]').blur();

                        Swal.fire({
                            icon: 'success',
                            title: 'Success!',
                            text: 'List of queued bills printed!',
                            focusConfirm: false,
                            focusCancel: false,
                            focusDeny: false,
                            customClass: {
