<script>
    ...
    $(document).ready(function() {
        ...
        $('#annual-amount').keyup(function() {
            ...
            annual_amnt = $(this).val().toString().split(".")[0].replace(/,/g, "");
            ...
            $('.monthly-percentage').each(function() {
                ...
                $(this).closest('tr').find('.monthly-amount').val(amount_limit.toLocaleString("en" , {minimumFractionDigits: 2 , maximumFractionDigits: 2}));
            });
        ...
        function percentage(curr_input) {
            ...
            percentage_tds.each(function () {
                ...
                let amnt_val = parseFloat(amount_input.val().split(".")[0].replace(/,/g, "")) || 0;
                var amnt_per_month = (per_val / 100) * $('#annual-amount').val();

                amount_input.val(amnt_per_month.toFixed(2));
            ...
            if (total_percentage > 100) {
                ...
                Swal.fire({
                    ...
                    $('#total-percentage').empty().text(percentage.toLocaleString("en" , {minimumFractionDigits: 2 , maximumFractionDigits: 2}));
                    $('#total-annual-amnt').text('').text(amount.toLocaleString("en" , {minimumFractionDigits: 2 , maximumFractionDigits: 2}));
                });
        ...
        $('#update-company-limit').click(function()  {
            var fields_array = [];
    ...
    $(document).ready(function() {
        ...
        $('#proceed-transaction').click(function() {
            ...
            if (sec_code_array.includes(user_sec_onpage)) {
                ...
                Swal.fire({
                    title: 'Success!',
                    text: 'Company limit configuration updated!',
                    icon: 'success',
                    timer: 900,
                    ...
                    var form_data = new FormData($('#update-company-limit-form')[0]);
                    form_data.append('CLIDs', CLIDs.join(", "));
                    form_data.append('CLDID', $('#CLDID').val());
                    ...
                    $.ajax({
                        url: "{{ route('maintenance.company_limit.update_company_limit') }}",
                        type: "post",
                        data: form_data,
                        contentType: false,
                        processData: false,
