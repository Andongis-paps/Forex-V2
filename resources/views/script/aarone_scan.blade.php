...
function processRequest() {
    ...
    switch ($action) {
        case 'upload': { // handle file upload
                ...
                print('<div id="main"><div id="left"><h5>Files Uploaded</h5>');
                ...
                print('</div>');
                print('</div><p id="file_front_id">' . $_FILES["com_asprise_scannerjs_images"]["name"][0] . '</p>');
                // print('</div><p id="file_cisform">' . $_FILES["com_asprise_scannerjs_images"]["name"][1] . '</p>');
...
function saveUploadedFileToDisk($name, $type, $tmp_name, $error, $size) {
    $dir = "\\\\192.168.188.122\\e\\wamp64\\www\\CMSFiles\\";
    ...
    // Open and write content to the file
    if (move_uploaded_file($tmp_name, $filePath)) {
        return 'File saved successfully: ' . $name;
    } else {
        return 'ERROR: Unable to save file!';
    }
    return dirname(getCurrentPageURL()) . str_replace(DIRECTORY_SEPARATOR, "/", substr($dir, strlen($_SERVER['DOCUMENT_ROOT'])));
}
...
function getCurrentPageURL() {
    ...
    $pageURL = "http://192.168.188.122/CMSFiles/";
