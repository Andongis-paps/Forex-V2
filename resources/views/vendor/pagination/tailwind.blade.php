{{-- @if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            Text for the pagination details
            <div>
...
@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
                <p class="text-xs text-gray-700 leading-5 my-0">
                    {!! __('Showing') !!}
                    @if ($paginator->firstItem())
                    ...
                    @endif
                    {!! __('of') !!}
                    <span class="font-extrabold underline">{{ $paginator->total() }}</span>
                    {!! __('results') !!}
                </p>
            </div>
            ...
            <div>
                <span class="relative z-0 inline-flex shadow-sm rounded-md">
                    ...
                    @if ($paginator->onFirstPage())
                        <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                            <span class="relative inline-flex items-center px-2 py-1 text-xs font-medium text-gray-500 bg-white border border-gray-300 cursor-default rounded-l-md leading-4" aria-hidden="true">
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="relative inline-flex items-center px-2 py-1 -ml-px text-xs font-medium text-white bg-[#566a7f] border border-gray-300 cursor-default leading-4">{{ $page }}</span>
                                    </span>
                                @else
                    ...
                    @else
                        <span aria-disabled="true" aria-label="{{ __('pagination.next') }}">
                            <span class="relative inline-flex items-center px-2 py-1 -ml-px text-xs font-medium text-gray-500 bg-white border border-gray-300 cursor-default rounded-r-md leading-4" aria-hidden="true">
                                <i class="bx bx-right-arrow-alt"></i>
                            </span>
                        </span>
                    @endif
