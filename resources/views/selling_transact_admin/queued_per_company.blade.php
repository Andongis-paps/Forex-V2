<html>
    <div style="border: 2px solid #000; padding-top: 3px; padding-bottom: 10px; padding-left: 4px; padding-right: 4px; margin-left: 10px; margin-top: 15px;">
        @php
            use Carbon\Carbon;

            $total_sum_peso = 0;
            $raw_date = Carbon::now('Asia/Manila');
        @endphp

        @forelse ($test as $selling_trans_details)
            @php
                $total_amnt_pesos = $selling_trans_details->total_bill_amount * $selling_trans_details->CMRUsed;
                $total_sum_peso += $total_amnt_pesos;
            @endphp

            {{-- <table class="selling-trans-details table table-bordered" style="display: none; padding: 0; width: 300pt;" cellspacing="0"> --}}
            <table class="selling-trans-details table table-bordered" style="display: none; padding: 3px; width: 300pt;" border=1 cellspacing="0">
                <tbody>
                    <tr class="t-0" style="margin: 0pt; padding: 0pt; border: 0px;">
                        <td class="p-0" style="font-size: 6pt; text-align: right; font-weight: 100; border: 0px;" colspan="5"><tt>{{ $selling_trans_details->Rset }}</tt></td>
                    </tr>
                    <tr class="t-0">
                        <td class="p-0" style="font-size: 12pt; text-align: center; font-weight: 700;" colspan="5"><b>{{ Str::title($selling_trans_details->CompanyName) }}</b></td>
                    </tr>
                </tbody>
                <tbody>
                    <tr>
                        <td class="p-0" style="font-size:9pt; text-align: center; padding: 1px;"><b>Currency</b></td>
                        <td class="p-0" style="font-size:9pt; text-align: center; padding: 1px;"><b>Amount</b></td>
                        <td class="p-0" style="font-size:9pt; text-align: center; padding: 1px;"><b>Quantity</b></td>
                        <td class="p-0" style="font-size:9pt; text-align: center; padding: 1px;"><b>Rate</b></td>
                        <td class="p-0" style="font-size:9pt; text-align: center; padding: 1px;"><b>Total</b></td>
                    </tr>
                </tbody>
                <tbody id="queued-bills-t-body">
                    <tr>
                        <td style="font-size: 8pt; text-align: center; padding: 0px;"><tt>{{ $selling_trans_details->Currency }}</tt></td>
                        <td style="font-size: 8pt; text-align: center; padding: 0px;"><tt>{{ number_format($selling_trans_details->total_bill_amount, 2,'.',',') }}</tt></td>
                        <td style="font-size: 8pt; text-align: center; padding: 0px;"><tt>{{ $selling_trans_details->total_bill_count }}</tt></td>
                        <td style="font-size: 8pt; text-align: center; padding: 0px;"><tt>{{ number_format($selling_trans_details->CMRUsed, 2,'.',',') }}</tt></td>
                        <td style="font-size: 8pt; text-align: right; padding-right: 5px;"><tt>{{ number_format($selling_trans_details->total_exchange_amount, 2,'.',',') }}</tt></td>
                    </tr>
                    <tr>
                        <td class="p-0" style="font-size: 8pt; text-align: right; padding-right: 5px;" colspan="4"><tt><b>Total</b></td>
                        <td class="p-0" style="font-size: 8pt; text-align: right; padding-right: 5px;" colspan="1"><tt><b>{{ number_format($total_sum_peso, 2,'.',',') }}</b></tt></td>
                    </tr>
                </tbody>
            </table>
            @empty
                Edi wow
        @endforelse
        <table class="selling-trans-details table table-bordered" style="display: none; padding: 0; width: 300pt; margin-top: 4px;" cellspacing="0">
            <tbody>
                <tr class="pt-0" style="text-align: left;">
                    <td class="p-0" style="font-size: 8pt;" colspan="1"><tt>{{ trans('labels.selling_admin_prep_by') }}:</tt></td>
                    <td class="p-0" style="font-size: 8pt; text-decoration: overline; text-align: center;" colspan="2"><tt>{{ Str::title(Auth::user()->Name) }}</tt></td>
                    <td class="p-0" style="font-size: 8pt;" colspan="1"><tt>{{ trans('labels.selling_admin_date') }}:</tt></td>
                    <td class="p-0" style="font-size: 8pt; text-align: center;" colspan="1"><tt>{{ $raw_date->toDateString() }}</tt></td>
                </tr>
                <tr class="pt-0" style="text-align: left;">
                    <td class="p-0" style="font-size: 8pt;" colspan="1"></td>
                    <td style="font-size: 8pt; text-align: center; border-top: 1px solid #000; padding: 0;" colspan="2"><tt><span style="text-decoration-line: overline; ">{{ trans('labels.selling_admin_signature') }}</span></tt></td>
                    <td class="p-0" style="font-size: 8pt;" colspan="1"></td>
                    <td class="p-0" style="font-size: 8pt; border-top: 1px solid #000;" colspan="1"></td>
                </tr>
            </tbody>
            <tbody>
                <tr class="pt-0" style="text-align: left;">
                    <td class="p-0" style="font-size: 8pt;" colspan="1"><tt>{{ trans('labels.selling_admin_received_by') }}:</tt></td>
                    <td class="p-0" style="font-size: 8pt; text-decoration: overline; text-align: center;" colspan="2"></td>
                    <td class="p-0" style="font-size: 8pt;" colspan="1"><tt>{{ trans('labels.selling_admin_date') }}:</tt></td>
                    <td class="p-0" style="font-size: 8pt;" colspan="1"></td>
                </tr>
                <tr class="pt-0" style="text-align: left;">
                    <td class="p-0" style="font-size: 8pt;" colspan="1"></td>
                    <td style="font-size: 8pt; text-align: center; border-top: 1px solid #000; padding: 0;" colspan="2"><tt><span style="text-decoration-line: overline; ">{{ trans('labels.selling_admin_signature') }}</span></tt></td>
                    <td class="p-0" style="font-size: 8pt;" colspan="1"></td>
                    <td class="p-0" style="font-size: 8pt; border-top: 1px solid #000;" colspan="1"></td>
                </tr>
            </tbody>
        </table>
    </div>
</html>
